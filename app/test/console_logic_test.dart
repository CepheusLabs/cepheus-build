...
void main() {
  ...
  // -------------------------------------------------------------------------
  // historyToCsv / csvField
  // -------------------------------------------------------------------------
  group('historyToCsv', () {
    BuildHistoryEntry entry({
      String id = '1',
      String command = 'bin/cepheus-build plan -p printdeck',
      int exitCode = 0,
    }) {
      return BuildHistoryEntry(
        ...
        store: '',
        command: command,
        startedAt: DateTime.utc(2026, 5, 30, 14, 22, 33),
        durationMs: 1500,
        exitCode: exitCode,
        output: '',
    ...
    test('emits a header row + one row per entry, CRLF terminated', () {
      final csv = historyToCsv([entry(id: 'a'), entry(id: 'b')]);
      final rows = csv.trimRight().split('\r\n');
      expect(rows.length, 3); // header + 2
      expect(rows.first.startsWith('id,started_at,product,'), isTrue);
      expect(csv.endsWith('\r\n'), isTrue);
    });

    test('quotes fields containing commas', () {
      final csv = historyToCsv([
        entry(command: 'build -p x --targets "a,b"'),
      ]);
      // the command field has a comma → must be wrapped in quotes
      expect(csv, contains('"build -p x --targets ""a,b"""'));
    });

    test('row carries status derived from exit code', () {
      final ok = historyToCsv([entry(exitCode: 0)]);
      final fail = historyToCsv([entry(exitCode: 1)]);
      expect(ok, contains(',complete,'));
      expect(fail, contains(',error,'));
    });
  });
  ...
  group('csvField', () {
    ...
    test('value with quote is escaped and wrapped', () {
