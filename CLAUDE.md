...
Cepheus Build is shared build and release orchestration for Cepheus Labs Flutter app products. It handles packaging, versioning, artifact collection, and store-release automation for Flutter apps with product-specific native pieces (Rust, Go, embedded systems). It is intentionally separate from Forge (shared UI design system, consumed by pinned git ref with generated local sibling overrides when needed).
...
```bash
./bin/cepheus-build deps -p cepheus-build --write   # optional local Forge override
cd app
flutter pub get
...
The committed app manifest depends on `package:forge` by pinned git ref; local Forge edits use the ignored sibling override generated by `deps --write`.
...
- **Build sync controls** — `--no-sync` skips the pre-build `git pull`; `--require-clean` aborts if the product working tree has uncommitted changes. Default is sync on, no clean check. `--dry-run` always skips sync regardless of `--no-sync`.
...
- Real local builds first auto-sync the product checkout with fast-forward-only `git pull` (`sync_repo_before_build()`). First-party dependencies are resolved from committed pins; local sibling checkout development uses `cepheus-build deps --write`. `--dry-run` does not mutate the checkout.
